...
address_dump='-'.join('{:02X}'.format(ord(b)) for b in address[::-1])
if len(address) < 2:
...
while True:
  ...
  if value[0] == 0:
    ...
    dump_data['data'].append({'ch':common.channels[channel_index],'lp':len(payload),'pl':':'.join('{:02X}'.format(b) for b in payload)})